				...
				if (!$current_timer OR $current_timer <= time()) {
					...
					if ( is_array( $api_1_1_content ) ) {
						...
						foreach ( $rss_i as $tweet ) {
							...
							$time_ago = ($i_creat!=false) ?  ' <a href="'. esc_url( $i_guid ) .'" target="'.$target4a.'" title="'.$the_time_ago['content'].'">' . $i_creat . '</a> ' . $the_time_ago['before'] : '<a href="'. esc_url( $i_guid ) .'" target="'.$target4a.'">' . $the_time_ago['content'] .'</a>';
